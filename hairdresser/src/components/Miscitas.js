...
const Miscitas = ({ clienteId }) => {
  ...
  const [citaIdToDelete, setCitaIdToDelete] = useState(null); //* me sobra pero antes de borrar comprobarlo bien
  const [citaCanceled, setCitaCanceled] = useState(false);
  const [clienteData, setClienteData] = useState({ nombre: '', apellido: '' }); // Para almacenar el nombre y apellido del cliente

  const navigate = useNavigate();

  useEffect(() => {
    const fetchCitas = async () => {
      // Obtenemos el nombre y apellido del cliente primero
      try {
